...
execFileSync("npm", ["install", "--prefix", prefix, "--cache", cache, "--ignore-scripts", path.join(tarballs, pack)], {stdio: "inherit"});
...
for (const args of [["--version"], ["--help"], ["list"], ["doctor"]]) {
  assertCommand(executable, args, fixture.environment, `installed piw ${args.join(" ")}`);
}
