...
export function ConfigApp({initial, entries, onSave, onCancel}: {initial: PiwStateV1; entries: Entry[]; onSave: (state: PiwStateV1) => void | Promise<void>; onCancel: () => void}) {
  ...
  return <Box flexDirection="column"><Text bold>{detail ? profile : "Configure Pi Profiles"}</Text>{saveError ? <Text color="red">{saveError}</Text> : <Text> </Text>}{detail ? visible.map((id, index) => { const selected = state.profiles[profile]!.entries.includes(id); const found = entries.find((entry) => entry.id === id); const color = index === entryIndex ? "cyan" : found?.status === "invalid" ? "red" : undefined; const description = found?.status === "valid" ? found.kind : found?.status === "invalid" ? `invalid: ${found.diagnostics[0]?.message ?? "unknown reason"}` : "missing"; return <Text key={id} {...(color ? {color} : {})}>{index === entryIndex ? "> " : "  "}[{selected ? (found?.status === "valid" ? "x" : "!") : " "}] {id} {description}</Text>; }) : names.map((name, index) => <Text key={name} {...(index === profileIndex ? {color: "cyan"} : {})}>{index === profileIndex ? "> " : "  "}{name}</Text>)}<Text dimColor>{detail ? "Space toggle  s save  Esc back" : "Enter edit  s save  q/Esc exit"}</Text></Box>;
}
